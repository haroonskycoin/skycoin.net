...
export default {
  ...
  home: {
    ...
    hero: {
      ...
      slider: {
        ...
        fiber: {
          title: 'Skycoin是Fiber',
          description: '一个无限可扩展、完全可定制的平台，具有类似Visa的速度和安全性。',
        },
      },
      buy: '购买Skycoin',
      wallet: {
    ...
    signup: {
      heading: '加入我们的Skyfleet通讯，和Skycoin一起到达新高度！',
  ...
  cx: {
    ...
    about: {
      heading: 'CX是什么？',
      subheading: '编程语言',
      paragraph1: 'CX是Skycoin的规范和编程语言，其中包含许多功能。CX可以改进Skycoin项目的诸多部分。为了增加安全性，CX允许创建复杂的合同协议。',
      paragraph2: 'CX将为在该平台上构建的项目部署可修改的功能。这些属性都是CX多功能编程语言的一部分：',
    },
    features: {
      heading: 'CX的特点：',
      affordances: {
        title: '功能可见性',
        text: '功能可见性允许许可包被特定用户同意或拒绝，从而有助于程序员安心地创建安全代码。可以通过查询可供性系统以获得可以引起相关功能的动作列表。',
      },
      programming: {
        title: '遗传编程（GP）',
        text: '遗传编程是在CX中创建程序时演变而来的。作为一种机器学习工具，GP将通过提供自动解决问题的引擎以及执行曲线拟合和创建预测模型的能力来帮助程序员。',
      },
      deterministic: {
        title: '确定性的语言',
        text: '确定性允许CX程序能够被可靠地创建，从而无需程序员关注嵌入式环境中的底层机器，因为无论机器如何都会产生输出。',
      },
      typing: {
        title: '严格的打字系统',
        text: 'CX严格的打字系统提供了另一层保护以及冗余和易用性，使调试过程简单明了。清晰的类型转换可以减少错误，并使CX对程序员从本质上来说是安全的。',
      },
      complied: {
        title: '编译和解释语言',
        text: '既可以作为编译语言，也可以作为解释语言。这使程序员能够利用其高级功能结合灵活和交互的附加功能。',
      },
      garbage: {
        title: '垃圾收集器',
        text: '作为一种自动内存管理形式，CX垃圾收集器会自动回收队列中未被使用的对象，从而释放宝贵的内存以进行必要的操作。CX在程序中查找将来无法访问的数据对象并将其处理掉。',
      },
      object: {
        title: '对象资源管理器',
        text: '程序员可以访问对象资源管理器，它将显示所有对象及其当前内存分配的图形视图。这些对象可以被序列化和传递，而代码步进将允许程序员在观察到的最后状态下继续执行这些对象。',
      },
    },
  ...
  fiber: {
    ...
    about: {
      heading: 'Fiber是什么？',
      subheading: '区块链3.0创新与平行链架构',
      quote: '“Skycoin的Fiber架构是一种转型，是优于区块链的解决方案，它使用分布式账本技术解决了现有问题。Fiber是区块链3.0的基础，可以最好地描述为一个无限可扩展的区块链网络，就像股线一样。这是一项技术壮举，比特币或以太坊等加密货币永远无法实现，因为它们从未被设计为无限扩展。“',
      author: '来自媒体',
      paragraph1: 'Fiber是Skycoin区块链平台的结构层，可定制，以适应任何区块链应用的需求。凭借创新且极其高效的代码，Fiber使Skycoin平台能够通过区块链解决方案适用于每个行业。',
      paragraph2: 'Fiber复杂而精妙，能够无限地扩展并满足多种类型分布式应用的需求。',
    },
    features: {
      heading: 'Fiber的优势',
      system: {
        title: '真正共生的平行链系统',
        text: 'Skycoin Fiber的架构可以横向和纵向扩展Skycoin平台，且不会因此大规模地影响网络独立性或造成网络缺陷。由于Skycoin的Obelisk共识机制，Fiber上的各个应用支持的代币将从根本上与其他代币隔离，使得各应用在网络稳定性，安全性或吞吐量方面永远不会相互影响。此外，通过Skycoin币时系统，每个使用Fiber的应用都将推动整个Skycoin平台前进，而不会妨碍现有的平台运营。Fiber在各个应用之间创造了前所未有的共生平衡，同时保持足够的隔离，使得任何一个应用都不会对网络的其余部分产生负面影响。',
      },
      forking: {
        title: '链可以分叉',
        text: '在Fiber上的所有代币将成为Skycoin核心代码的分支，这意味着每个应用都将具有与Skycoin相同级别的高级功能，吞吐量与安全性。',
      },
      blockchain: {
        title: '定制区块链',
        text: '在Skycoin Fiber上运行的项目将拥有自己完全独立且高度可定制的区块链，以满足不同项目的独特需求。寄希望于一组规则或操作能满足不同项目的要求，从根本上说，这是一种很天真的想法。但这也意味着，如果区块链平台要成功地纵向和横向扩展，适应性是平台中最关键的因素。',
      },
      token: {
        title: '真正的代币支持',
        text: '基于Skycoin Fiber架构构建的项目将具有前所未有的网络支持水平。项目将能够在我们开创性的点对点网络Skywire上运行，每个项目将在Skycoin生态系统中立即获得钱包和交易支持。没错，项目可以在几分钟内在我们的世界级平台上启动和交易。',
      },
      operation: {
        title: '独立运作',
        text: '随着Skywire发展成为全球点对点网状网络，在Fiber上运行的应用将完全独立于对互联网服务提供商等传统网络基础设施的依赖。这种平台独立性是Skycoin的Fiber及其分布式应用程序和项目所独有的。',
      },
    },
  },
  skyminerForm: {
  ...
  obelisk: {
    title: 'Obelisk是什么',
    description: '革命性的SKYCOIN共识算法',
    about: {
      heading: 'Obelisk是什么？',
      subheading: '革命性的SKYCOIN共识算法',
      quote: '“中心化的问题，51％的攻击，挖矿问题，缓慢的交易速度和能源浪费是旧的共识算法面临的一些关键缺点。为了解决这些问题，Skycoin开发了Obelisk，这是一种新的分布式共识算法，可以解决前几代共识算法的问题。”',
      author: '来源于媒体',
      paragraph1: 'Obelisk是一种革命性的共识算法，基于“信任之网”，旨在消除前几代共识算法给区块链带来的问题。信任之网架构创建了一个完美的检查和平衡系统，使社区和其他节点能够高度准确地审核网络行为。',
      paragraph2: '由于能够切断可疑节点的恶意行为，网络会随着信任的增强而变得越来越强大。节点的隔离可以管理信任，同时，此过程允许社区重新平衡网络中的能力。',
    },
    features: {
      heading: 'Obelisk的特点',
      consumption: {
        title: '低能耗和高度可扩展性',
        text: 'Obelisk共识算法是可扩展的且计算成本低廉, 是工作量证明（PoW）的替代算法。Obelisk使算法和区块创建能够在预算开源的硬件上运行。',
      },
      attack: {
        title: '抵御51%攻击',
        text: '网络信任共识阻碍了中心化权力的发展。Skycoin不依赖采矿激励，因此不会受到相同的PoW / PoS漏洞的影响。',
      },
      defence: {
        title: '有力地抵御协同攻击',
        text: 'Obelisk可以抵御大规模、有组织的恶意节点网络协同攻击。该算法是非迭代的，能快速收敛，可以在只有近邻连接的稀疏的网络（如网状网络）上运行，也能在连接图 （即不需要DAG-type连接）的周期里很好地运行。',
      },
      structure: {
        title: '存在两种类型的节点 ',
        text: '一个共识节点从一个或多个成块节点接收输入。每个节点的算法是独立的，但它们都在相同的数据结构上运行。两种类型节点一直对数据来源进行验证，检测欺诈性数据。欺骗性的或无效的消息被检测到，会被除掉，且永远不得传播——参与可疑活动的对等节点会被切断，其公钥也被禁止使用。',
      },
    },
  },
  gallery: {
